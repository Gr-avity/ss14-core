alerts-shadekin-name = Воздействие света
alerts-shadekin-desc = Сколько света вокруг вас.
alerts-shadekin-energy-name = Энергия
alerts-shadekin-energy-desc = Ваш уровень энергии.
alerts-shadekin-noportal-name = Отсутствует портал
alerts-shadekin-noportal-desc = [color=red]У вас нет портала, вы не сможете омолаживаться![/color]
alerts-shadekin-rejuvenate-name = Омоложение
alerts-shadekin-rejuvenate-desc = Вы в настоящее время омолаживаетесь, вы не сможете покинуть тьму, пока не восстановите полную энергию.
roles-antag-brighteye-name = Светлоглазый
roles-antag-brighteye-description = Посетитель и вмешивающийся из «Тьмы». Вмешивается и наблюдает оттуда, куда свет не достигает.
objective-issuer-brighteye = [color=#6c15ae]Светлоглазый[/color]
brighteye-role-greeting =
    You are a Bright-eye!
    You are a Shadekin from the Dark, a creature of the shadows that is foreign and alien to this dimension.
    Explore and exert your influence over where the light does not reach, breach the spaces between this one and the dark with your portal and learn of this world.
shadekin-portal-owner = Это ваш портал
shadekin-portal-stability-stable = Текущее состояние: [color=forestgreen]Стабильно[/color]
shadekin-portal-stability-unstable = Текущее состояние: [color=crimson]Растёт[/color]
shadekin-portal-health-percentage = Текущее здоровье: [color=red]{ $percent }[/color]
shadekin-portal-destroy = [color=red]Уничтожить портал[/color]
shadekin-portal-stabilize = Стабилизировать портал
shadekin-portal-stabilize-info = Стабилизируйте и лечите ваш портал, используя вашу энергию. (Стоит 50 энергии)
shadekin-noenergy = Недостаточно энергии для выполнения этого действия.
shadekin-fail-generic = Я не могу использовать свою энергию!
phase-fail-generic = Я не могу фазировать!
hubportal-rejuvenate = Вы не можете использовать это так скоро после экстренного варпа!
shadekin-rejuvenate-compleated = Вы чувствуете, что можете снова покинуть Тьму
shadekin-shunt = Что-то резонирует с вашим ядром и ставит вас на колени.
shadekin-too-bright = Свет слишком интенсивен здесь!
shadekin-core-undamaged = [color=yellow]Ядро наполнено силой![/color]
shadekin-core-owner = [color=yellow]Это ядро принадлежит вам.[/color]
brighteye-thedark = [bold]Тьма[/bold]
brighteye-darktiles = [color=red]{ $darkCount }[/color] тёмных плиток на станции.
brighteye-darkstation = станция [color=red]не[/color] была перемещена в тьму.
admin-verb-text-make-brighteye = Сделать Светлоглазым
admin-verb-make-brighteye = Сделать цель Светлоглазым.
shadekinTrap-trigger = Набор кристаллов выскакивает из земли, и теневые щупальца начинают обвивать { $user }
shadekinTrap-trigger-fail = Теневые щупальца не могут ничего поймать и рассеиваются.
alerts-shadekin-thedark-name = Тьма
alerts-shadekin-thedark-desc = Вы в Тьме... и без защиты или не будучи шейдкином... Это влияет на вас.
dark-breacher-window-title = Прорыватель Тьмы

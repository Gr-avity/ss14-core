ent-CrateEmergencyExplosive = ящик сапёрного снаряжения
    .desc = Science gone bonkers? Beeping behind the airlock? Buy now and be the hero the station des... I mean needs! (time not included)
ent-CrateEmergencyFire = ящик пожарного снаряжения
    .desc = Only you can prevent station fires. Partner up with two firefighter suits, gas masks, flashlights, large oxygen tanks, extinguishers, hardhats, and emergency cooling units!
ent-CrateEmergencyInternals = ящик аварийного снаряжения
    .desc = Master your life energy and control your breathing with three breath masks, three gas masks, three emergency EVA suits, three oxygen tanks, three nitrogen tanks, and 3 cooling units.
ent-CrateEmergencyInternalsLarge = ящик внутренних систем (большой)
    .desc = Master your life energy and control your breathing with six breath masks, six gas masks, six emergency EVA suits, six oxygen tanks, six nitrogen tanks, and six cooling units.
ent-CrateNitrogenInternals = ящик со снаряжением для дыхания (азот)
    .desc = Contains two breath masks, two gas masks, and four nitrogen tanks. Intended for nitrogen-breathing species.
ent-CrateEmergencyRadiation = ящик противорадиационного снаряжения
    .desc = Survive the Nuclear Apocalypse and Supermatter Engine alike with two sets of Radiation suits. Each set contains a helmet, suit, and Geiger counter. We'll even throw in a bottle of vodka and some glasses too, considering the life-expectancy of people who order this.
ent-CrateEmergencyInflatablewall = ящик надувных стен
    .desc = Three stacks of inflatable walls for when the stations metal walls don't want to hold atmosphere anymore.
ent-CrateGenericBiosuit = ящик биокостюмов
    .desc = Contains 2 biohazard suits to ensure that no disease will distract you from what you're doing there.
